...
| `direction` | `"left"` \| `"right"` \| `"above"` \| `"below"` | any node | Soft placement preference for **this node's successor(s)** — "whatever comes after me, put it below/above/etc. me." A successor's own `placement` (below) always wins if both are set ("child overrides parent"). Only `above`/`below` are meaningful in a horizontal flow, only `left`/`right` in a vertical one. |
| `placement` | `"left"` \| `"right"` \| `"above"` \| `"below"` | any node | This node's OWN placement preference relative to whichever predecessor points at it — the mirror image of `direction`. Wins over a predecessor's `direction` hint for the same edge. If multiple predecessors offer conflicting `direction` hints and this node has no `placement` of its own, the engine picks one and logs a warning rather than guessing silently. |
